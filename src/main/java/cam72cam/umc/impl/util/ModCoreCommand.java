...
import cam72cam.umc.impl.text.PlayerMessage;
...
public class ModCoreCommand extends Command {
	...
	private boolean sendChunkInfo(World world, Consumer<PlayerMessage> sender, Optional<Player> player, List<String> args) {
		...
		for (Chunk chunk : chunks) {
			...
			long umcCount = chunk.getTileEntityMap().values().stream().filter(x -> x instanceof cam72cam.umc.impl.block.tile.TileEntity).count();
			...
			if (all || isChunkLocation || !hasChunkLocation && (teCount > 0 || entityCount > 0)) {
				...
				if (debug) {
					...
					for (TileEntity tile : chunk.getTileEntityMap().values()) {
						...
						if (tile instanceof cam72cam.umc.impl.block.tile.TileEntity) {
							BlockEntity instance = ((cam72cam.umc.impl.block.tile.TileEntity) tile).instance();
